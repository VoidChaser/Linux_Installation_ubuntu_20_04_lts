...
Для быстрого воспроизведения окружения:
```bash
git clone https://github.com/VoidChaser/Linux_Installation_ubuntu_20_04_lts
cd Linux_Installation_ubuntu_20_04_lts
sudo bash setup.sh
```
...
```bash
curl -sSL https://raw.githubusercontent.com/VoidChaser/Linux_Installation_ubuntu_20_04_lts/main/setup.sh | bash
```
...
## Сложности и решения
**1. SSH connection refused после смены порта**
- **Проблема:** `ssh: connect to host X port 2022: Connection refused`
- **Решение:**
```bash
...
# Проверить firewall (если используется)
sudo ufw allow 2022/tcp
sudo ufw reload
```

**2. Конфликт статического IP с DHCP-сервером VirtualBox**
...
**3. Swap не отображается в fdisk как раздел**
...
**4. Удаление cookie в FastAPI (контекст из другого проекта, но принцип применим)**
...
**5. Netplan apply зависает при загрузке**

- **Проблема: Система долго грузится из-за ожидания сети**

- **Решение: Добавить optional: true в /etc/netplan/*.yaml:**
```bash
text
network:
  ethernets:
    enp0s3:
      ...
      optional: true  # Не ждать полной инициализации сети
```

**6. CRON задачи не выполняются**

- **Проблема: Задача в crontab -l но не запускается**
...
# Убедиться что используется полный путь
which <command>  # Узнать путь к команде
# Использовать в crontab: */5 * * * * /usr/bin/<command>
```
...
- **Enterprise Linux окружения:** Опыт настройки защищенных дистрибутивов Linux и управления пользователями применим в проектах с повышенными требованиями к безопасности и соответствию стандартам информационной безопасности.
...
| Категория | Технические навыки | Применение в DevOps |
|-----------|-------------------|---------------------|
| **Networking** | Static IP (netplan), DNS config, gateway routing | Подготовка инфраструктуры для K8s/Docker Swarm |
| **Security** | SSH hardening (port 2022), sudo management, user privileges | Безопасная настройка CI/CD runners, production серверов |
| **System Admin** | systemd services, NTP sync, log analysis (syslog, auth.log) | Диагностика инцидентов, управление сервисами |
| **Monitoring** | top, htop, ncdu, fdisk, df, du | Resource optimization, capacity planning |
| **Automation** | CRON scheduling, bash scripting | Автоматизация backup, maintenance задач |
| **Documentation** | Technical writing, diagramming (Mermaid + ASCII) | Knowledge sharing, onboarding новых сотрудников |

**Real-world применение:**
- Базовая настройка нод для контейнерных оркестраторов (Kubernetes, Docker Swarm)
- Подготовка окружений для self-hosted CI/CD (GitLab Runner, Jenkins agents)
- Системный мониторинг и capacity planning production-серверов
- Автоматизация routine maintenance через CRON + Ansible
...
Этот проект — **фундамент** для более сложных DevOps-практик. Следующие шаги:

### Immediate Next Level
- 🐳 **[Docker Containerization](#)** — Контейнеризация приложений, Docker Compose
- 🔄 **[CI/CD Pipeline](#)** — GitHub Actions/GitLab CI для автоматического деплоя
- 📊 **[Monitoring Stack](#)** — Prometheus + Grafana + Node Exporter

### DevOps Learning Path
```markdown
1. ✅ Linux Basics (этот проект)
2. 🐳 Docker & Containerization
3. 🔄 CI/CD Automation
4. ☸️ Kubernetes Orchestration
5. 🏗️ Infrastructure as Code (Terraform/Ansible)

```

### Technologies to Learn Next
- **Config Management:** Ansible для автоматизации настройки серверов
- **Containers:** Docker, Docker Compose, Podman
- **Orchestration:** Kubernetes, Docker Swarm
- **IaC:** Terraform для provisioning инфраструктуры
- **Monitoring:** Prometheus, Grafana, ELK Stack

## Contributing

Нашли ошибку или хотите улучшить документацию? Pull requests приветствуются!
